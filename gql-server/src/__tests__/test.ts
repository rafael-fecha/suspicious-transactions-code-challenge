...
it('should get only the suspicious transactions', async () => {
  ...
  const {  data: { transactionSuspicious }} = await query({ query: gql`
  query {
...
it('should mark the transaction as blocked', async () => {
  ...
  const { data: { markTransactionSuspicious }} = await mutate({ mutation: gql`
...
`, variables: { transactionData: { id: 1, status: "Blocked" } } });
  ...
  console.log(markTransactionSuspicious)
